...
if [ -f /etc/debian_version ]; then
   if [ -z "$(grep -i "follow symlinks" /etc/samba/smb.conf)" ]; then
      sudo sed -i '/\[global\]/a         follow symlinks = yes' /etc/samba/smb.conf
   fi
   if [ -z "$(grep -i "wide symlinks" /etc/samba/smb.conf)" ]; then
      sudo sed -i '/\[global\]/a         wide symlinks = yes' /etc/samba/smb.conf
   fi
...
if [ -z "$(grep -i "usershare allow" /etc/samba/smb.conf)" ]; then
   sudo sed -i '/\[global\]/a         usershare allow guests = yes' /etc/samba/smb.conf
fi
if [ -z "$(grep -i "locking = yes" /etc/samba/smb.conf)" ]; then
   sudo sed -i '/\[global\]/a         locking = yes' /etc/samba/smb.conf
fi
if [ -z "$(grep -i "kernel oplocks" /etc/samba/smb.conf)" ]; then
   sudo sed -i '/\[global\]/a         kernel oplocks = yes' /etc/samba/smb.conf
fi
...
if [ -z "$(grep -i "socket options" /etc/samba/smb.conf)" ]; then
   sudo sed -i '/\[global\]/a         socket options = TCP_NODELAY SO_RCVBUF=8192 SO_SNDBUF=8192' /etc/samba/smb.conf
fi
